    ...
    <div className="flex items-center space-x-2 text-sm text-gray-500 mt-2">
      {post.authorAvatar ? (
        ...
        <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center">
          <span className="text-indigo-700 font-medium">{post.author.charAt(0).toUpperCase()}</span>
        </div>
      )}
    ...
    <main className="py-28 pb-12 px-6 max-w-7xl mx-auto">
      ...
      <section className="mb-16">
        <Link href={`/blogs/${hero.slug}`} className="group block overflow-hidden rounded-xl shadow hover:shadow-xl transition">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <img src={hero.image} className="w-full h-80 object-cover rounded-xl" alt={hero.title} />
            <div className="p-6">
              <h2 className="text-3xl font-bold text-indigo-700 mb-4 group-hover:text-indigo-600">{hero.title}</h2>
              <p className="text-gray-600 text-lg mb-4">{hero.excerpt || stripHtml(hero.content).slice(0, 200) + "…"}</p>
              {renderMeta(hero)}
            </div>
          </div>
        </Link>
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {trending.map((post) => (
            <Link key={post.slug} href={`/blogs/${post.slug}`} className="group rounded-xl bg-white shadow hover:shadow-lg transition">
              <img src={post.image} className="w-full h-40 object-cover rounded-t-xl" alt={post.title} />
              <div className="p-4">
                <h4 className="font-semibold text-lg text-indigo-700 group-hover:text-indigo-600 mb-2">{post.title}</h4>
                <p className="text-sm text-gray-600 line-clamp-2">{stripHtml(post.excerpt)}</p>
                {renderMeta(post)}
              </div>
            </Link>
          ))}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {gridPosts.map((post) => (
            <Link key={post.slug} href={`/blogs/${post.slug}`} className="group block overflow-hidden rounded-xl bg-white shadow hover:shadow-md transition">
              <img src={post.image} className="w-full h-48 object-cover" alt={post.title} />
              <div className="p-4">
                <h4 className="text-lg font-semibold text-indigo-800 group-hover:text-indigo-600 mb-2">{post.title}</h4>
                <p className="text-sm text-gray-600 line-clamp-2">{stripHtml(post.excerpt)}</p>
                {renderMeta(post)}
              </div>
            </Link>
